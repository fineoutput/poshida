...
class Cart extends CI_finecontrol
{
    ...
    public function get_courier_serviceability()
    {
        $this->load->helper(array('form', 'url'));
        $this->load->library('form_validation');
        $this->load->helper('security');
        if ($this->input->post()) {
            $this->form_validation->set_rules('sub_total', 'sub_total', 'required|xss_clean|trim');
            $this->form_validation->set_rules('total_weight', 'total_weight', 'required|xss_clean|trim');
            $this->form_validation->set_rules('pincode', 'pincode', 'required|xss_clean|trim');
            if ($this->form_validation->run()== true) {
                $sub_total=$this->input->post('sub_total');
                $total_weight=$this->input->post('total_weight');
                $pincode=$this->input->post('pincode');
                $shipping=$this->shiprocket->GetCourierServiceability($pincode, $total_weight,$sub_total);
                echo $shipping;
            } else {
